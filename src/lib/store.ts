...
export function reducer(state: BoardState, action: Action): BoardState {
  switch (action.type) {
    ...
    case 'update':
      return mapActive(state, (items) =>
        items.map((i) => (i.id === action.id ? touch({ ...i, ...action.patch }) : i)),
      );
    case 'add-note': {
      const note: Note = { id: uid(), text: action.text, createdAt: Date.now() };
      return mapActive(state, (items) =>
        items.map((i) => (i.id === action.id ? touch({ ...i, notes: [...i.notes, note] }) : i)),
      );
    }
    case 'delete-note':
      return mapActive(state, (items) =>
        items.map((i) =>
          i.id === action.id ? touch({ ...i, notes: i.notes.filter((n) => n.id !== action.noteId) }) : i,
        ),
      );
    case 'delete':
...
function load(): BoardState {
  try {
    ...
    // migrate the single-board v1 format into a default project
